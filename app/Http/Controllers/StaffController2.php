...
class StaffController extends Controller
{
    ...
    public function register(Request $request){
        ...
        return redirect('staff/newcutomer');
        ...
        'description' => 'required',
    			...
    			if ($request->input('type')=='loan') {
    				...
    				if ( !isset($loan) && !isset($pending)) {
                        ...
                        if ($latest_loan->week_due_date->diffInWeeks($latest_loan->due_date) >= 0 && $latest_loan->interest_status == 'paid' ) {
    ...
    public function viewcustomer($id){
    	...
    	if (Auth::user()->role == 'staff' && $user) {
    		...
    		if ($user->role == 'customer') {
    		 	...
    		 	return view('staff.viewcustomer')->with(['user' => $user, 'loan' => $loan, 'historys' =>$historys, 'rejected' => $rejected]);
    		 }else {
