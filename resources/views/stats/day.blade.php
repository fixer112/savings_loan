...
$total_approved = $data['His']->where('approved', 'yes')->where('updated_at', '>=', $today)->get();


$total_loan = $data['His']->whereIn('user_id', $users)->where('approved', 'yes')->where('updated_at', '>=', $today)->where('type', 'loan')->get();
$total_loan_sum = $total_loan->sum('debit');

$total_withdraw = $data['His']->whereIn('user_id', $users)->where('approved', 'yes')->where('updated_at', '>=', $today)->where('type', 'withdraw')->get();
$total_withdraw_sum = $total_withdraw->sum('debit');

$total_deposit = $data['His']->whereIn('user_id', $users)->where('approved', 'yes')->where('updated_at', '>=', $today)->where('type', 'deposit')->get();
$total_deposit_sum = $total_deposit->sum('credit');

$total_interest = $data['His']->whereIn('user_id', $users)->where('approved', 'yes')->where('updated_at', '>=', $today)->where('type', 'interest_fee')->get();
$total_interest_sum = $total_interest->sum('debit');

$total_default = $data['His']->whereIn('user_id', $users)->where('approved', 'yes')->where('updated_at', '>=', $today)->where('type', 'default_fee')->get();
    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            <div class="card">
                ...
                <div class="card-header"><strong>Today's Statistics</strong></div>
